...
export default function Appointment() {
  ...
  useEffect(() => {
    if (selectedDate) {
      const userListRef = collection(db, "appointments");
      const q = query(userListRef, where("date", "==", selectedDate.format("YYYY-MM-DD")));
      ...
      const unsubscribeSnapshot = onSnapshot(
        ...
        (querySnapshot) => {
          ...
          querySnapshot.forEach((doc) => {
            ...
            setPrevAppointments(prev => [...prev, details]);
          });
          setLoadingTimes(false);
        },
        (error) => {
          console.error("Error fetching appointments");
          toast.error("Error fetching times, Please try again", {
            style: {
              borderRadius: "10px",
              background: "#333",
              color: "#fff",
  ...
  const renderTimes = () => {
    const day = selectedDate && getSelectedDay(selectedDate);
    const currentDate = dayjs();
    const isToday = selectedDate && selectedDate.isSame(currentDate, "day");
  
    return availability[day].map((time, index) => {
      const [hour, minute, period] = time.split(/[:\s]/);
      let time24 = parseInt(hour, 10);
      if (period === "PM" && time24 !== 12) {
        time24 += 12;
      } else if (period === "AM" && time24 === 12) {
        time24 = 0;
      }
  
      const timeDate = selectedDate.hour(time24).minute(parseInt(minute, 10));
      const isPast = isToday && timeDate.isBefore(currentDate);
      ...
      return (
        <Button
          ...
          style={{
            margin: "0.5em",
            borderColor: (isPast || prevAppointments?.find((e) => e?.time === time) || loadingTimes) ? "#7e7e7e" : "#ccc",
            color: selectedTime === time ? "#000" : (isPast || prevAppointments?.find((e) => e?.time === time) || loadingTimes) ? "#8e8e8e" : "#fff",
          }}
          disabled={isPast || prevAppointments?.find((e) => e?.time === time) || loadingTimes}
        >
          {time}
        </Button>
      );
  ...
  return (
    <section ref={appointmentRef} className={classes.sectionContainer}>
      ...
      >
        {/* <div className={classes.colHeading} style={{ marginBottom: "10px" }}>
